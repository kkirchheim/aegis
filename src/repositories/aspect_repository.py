...
class AspectRepository:
    ...
    def get_default_aspects() -> List[Aspect]:
        ...
        try:
            return list(Aspect.select().where(Aspect.is_default).order_by(Aspect.created_at))
        except Exception:
            return []

    @staticmethod
    def create_aspect(name: str, description: str, prompt: str, is_default: bool = False) -> Aspect:
        """Create a new aspect."""
        return Aspect.create(name=name, description=description, prompt=prompt, is_default=is_default)

    @staticmethod
    def update_aspect(
        aspect_id: UUID, name: Optional[str] = None, description: Optional[str] = None, prompt: Optional[str] = None
    ) -> Optional[Aspect]:
...
class UserAspectRepository:
    ...
    def get_user_aspects(user_id: Union[int, UUID]) -> List[UserAspect]:
        ...
        try:
            return list(UserAspect.select().where(UserAspect.user_id == user_id).order_by(UserAspect.created_at))
        except Exception:
            return []
    ...
    def get_active_aspects(user_id: Union[int, UUID]) -> List[Aspect]:
        ...
        try:
            return list(
                Aspect.select()
                .join(UserAspect)
                .where((UserAspect.user_id == user_id) & (UserAspect.is_active) & (UserAspect.deleted_at.is_null()))
    ...
    @staticmethod
    def get_user_aspect(user_id: Union[int, UUID], aspect_id: Union[str, UUID]) -> Optional[UserAspect]:
        """Get specific user aspect."""
        try:
            return UserAspect.get((UserAspect.user_id == user_id) & (UserAspect.aspect_id == aspect_id))
        except UserAspect.DoesNotExist:
            return None

    @staticmethod
    def create_user_aspect(
        user_id: Union[int, UUID], aspect_id: Union[str, UUID], custom_prompt: Optional[str] = None
    ) -> UserAspect:
        """Create user aspect entry."""
        return UserAspect.create(user_id=user_id, aspect_id=aspect_id, custom_prompt=custom_prompt, is_active=True)
