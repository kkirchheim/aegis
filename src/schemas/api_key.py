...
class APIKeySchema(Schema):
    ...
    id = fields.Str(description="Unique API key identifier")
    name = fields.Str(description="Human-readable name for this key")
    key_prefix = fields.Str(description="First 8 characters of key (safe to display in logs/UI)")
    created_at = fields.DateTime(description="When this key was generated")
    last_used_at = fields.DateTime(allow_none=True, description="When this key was last used for authentication")
    is_active = fields.Bool(description="Whether this key is currently valid")
...
class APIKeyGenerateSchema(Schema):
    ...
    id = fields.Str(description="Unique API key identifier")
    name = fields.Str(description="Human-readable name for this key")
    key = fields.Str(description="FULL API KEY - shown only once! Save this securely.")
    key_prefix = fields.Str(description="First 8 characters of key")
    created_at = fields.DateTime(description="When this key was generated")
...
class APIKeyListSchema(Schema):
    ...
    keys = fields.List(fields.Nested(APIKeySchema), description="List of API keys")
    total = fields.Int(description="Total number of API keys")
