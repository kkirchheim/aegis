...
class AspectCreateSchema(Schema):
    ...
    name = fields.Str(
        ...
        description="Aspect name (1-255 characters)"
    )
...
class AspectUpdateSchema(Schema):
    ...
    description = fields.Str(
        required=False,
        allow_none=False,
        description="Updated aspect description"
    )
    prompt = fields.Str(
        required=False,
        allow_none=False,
        description="Updated evaluation prompt template"
    )
...
class ActivateAspectSchema(Schema):
    ...
    is_active = fields.Bool(
        required=True,
        description="Whether aspect should be active"
    )
...
class OverridePromptSchema(Schema):
    ...
    custom_prompt = fields.Str(
        required=False,
        allow_none=True,
        description="Custom prompt override (null to revert to default)"
    )
...
class AspectSchema(Schema):
    ...
    """
    id = fields.UUID(
        description="Unique aspect identifier"
    )
    name = fields.Str(
    ...
    is_default = fields.Bool(
        description="Whether this is a default system aspect"
    )
    created_at = fields.DateTime(
        description="When aspect was created"
    )
    updated_at = fields.DateTime(
        description="When aspect was last updated"
    )
...
class UserAspectSchema(Schema):
    ...
    """
    id = fields.UUID(
        description="Unique user-aspect identifier"
    )
    aspect_id = fields.UUID(
        description="Reference to the global aspect"
    )
    name = fields.Str(
        description="Aspect name"
    )
    ...
    is_active = fields.Bool(
        description="Whether this aspect is active for the user"
    )
    custom_prompt = fields.Str(
        allow_none=True,
        description="Custom prompt override (null if using default)"
    )
    prompt_to_use = fields.Str(
        description="The actual prompt to use (custom or default)"
    )
    created_at = fields.DateTime(
        description="When user activated this aspect"
    )
...
class AspectListSchema(Schema):
    ...
    """
    aspects = fields.List(
        fields.Nested(UserAspectSchema),
        description="List of user aspects"
    )
    total = fields.Int(
        description="Total number of aspects for user"
    )
