...
class ScriptService:
        ...
        for script in ExecutionScript.select():
            ...
            try:
                user_script = UserScript.get(
                    UserScript.user_id == user_id, UserScript.script_hash == script.script_hash
                )
            ...
            scripts.append(
                {
                    ...
                    "script_text_preview": script.script_text[:200]
                    if len(script.script_text) > 200
                    else script.script_text,
                    "created_by": script.created_by.username if script.created_by else "System",
                    "is_active": is_active,
                    "is_default": is_default,
                    "is_owned": is_owned,
                    "created_at": script.created_at.isoformat()
                    if hasattr(script.created_at, "isoformat")
                    else str(script.created_at),
                }
            )
    ...
    def get_active_scripts_for_user(user_id: int) -> List[Dict[str, Any]]:
        ...
        for user_script in UserScript.select().where(UserScript.user_id == user_id, UserScript.is_active):
            try:
    ...
    def deactivate_script(user_id: int, script_hash: str) -> bool:
        ...
        """
        try:
            user_script = UserScript.get(UserScript.user_id == user_id, UserScript.script_hash == script_hash)
    ...
    def create_user_script(
        ...
        """
        from utils.script_utils import get_or_create_script
