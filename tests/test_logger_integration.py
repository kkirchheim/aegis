...
"""
        ...
        # Call evaluate_paper with real function logger
...
"""
    ...
    def test_stage3_evaluation_passes_logger_wrapper_to_evaluate_paper(self, test_job, test_user, test_paper_analysis, test_execution_details):
        ...
        with patch('services.pipeline_orchestrator.evaluate_paper', side_effect=mock_evaluate_paper):
            with patch('services.pipeline_orchestrator.EventDispatcher'):
                result = stage_3_evaluation(test_job.id, func_logger)
        
        assert result == True
        assert passed_logger is not None
        # Verify it's not the raw function
        assert not callable(passed_logger), "Should not pass raw function to evaluate_paper"
    
    def test_logger_wrapper_has_required_methods(self, test_job, test_user, test_paper_analysis, test_execution_details):
