...
def test_error_handling():
    ...
    for status, expected_stage in error_states.items():
        assert status == "failed", "Error status should be 'failed'"
        assert expected_stage == "failed", "Error stage should be 'failed'"
...
def test_evaluation_stage_persistence():
    ...
    for stage_name, expected_current, expected_status in stages_sequence:
        if stage_name == "stage_3_complete":
            ...
            assert expected_current == "evaluation", "BUG: Evaluation stage closing too early on stage_3_complete"
            assert expected_status == "processing", "BUG: Job status should still be processing"
