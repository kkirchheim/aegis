...
from utils.check_utils import get_or_create_check, hash_script, seed_default_checks
...
class TestChecksMVP:
    ...
    def test_seed_default_checks(self, app):
        ...
        with app.app_context():
            ...
            readme_check = next((s for s in checks if s.name == "README Exists"), None)
    ...
    def test_check_result_storage(self, app):
        ...
        with app.app_context():
            ...
            user = User.create(username="testuser", email="test@example.com", password_hash="hash")

            job = Job.create(id=str(uuid4()), user=user, pdf_path="/tmp/test.pdf", status="processing")
    ...
    def test_check_result_retrieval_by_job(self, app):
        ...
        with app.app_context():
            ...
            results = list(CheckResult.select().where(CheckResult.job == job.id))
