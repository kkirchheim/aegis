...
class TestAdminUserStatus:
    ...
    def test_activate_user(self, admin_user, inactive_user):
        ...
        response = admin_user.patch(f"/api/admin/users/{inactive_user['id']}", json={"is_active": True})
    ...
    def test_deactivate_user(self, admin_user, active_user):
        ...
        response = admin_user.patch(f"/api/admin/users/{active_user['id']}", json={"is_active": False})
    ...
    def test_update_nonexistent_user(self, admin_user):
        ...
        response = admin_user.patch("/api/admin/users/99999", json={"is_active": True})

        assert response.status_code == 404

    def test_update_user_non_admin(self, authenticated_user, inactive_user):
        """Test non-admin can't update user status"""
        response = authenticated_user.patch(f"/api/admin/users/{inactive_user['id']}", json={"is_active": True})
