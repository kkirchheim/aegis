...
class TestChatSendMessage:
    ...
    def test_send_message_success(self, authenticated_user, test_job):
        ...
        response = authenticated_user.post(
            f"/api/job/{test_job['id']}/chat", json={"message": "What are the main findings of this paper?"}
        )
    ...
    def test_send_message_empty(self, authenticated_user, test_job):
        ...
        response = authenticated_user.post(f"/api/job/{test_job['id']}/chat", json={"message": ""})
    ...
    def test_send_message_nonexistent_job(self, authenticated_user):
        ...
        response = authenticated_user.post("/api/job/nonexistent/chat", json={"message": "Test"})

        assert response.status_code == 404

    def test_chat_requires_auth(self, client, auth_test_job):
        """Test chat requires authentication"""
        response = client.post(f"/api/job/{auth_test_job['id']}/chat", json={"message": "Test"})
...
class TestChatPermissions:
    ...
    def test_other_user_cant_access_chat(self, authenticated_user, other_user, test_job):
        ...
        authenticated_user.post(f"/api/job/{test_job['id']}/chat", json={"message": "Test message"})
