...
class TestUserPluginRepository:
    ...
    def test_multiple_users_isolated(self, app):
        ...
        with app.app_context():
            ...
            UserPluginRepository.create_user_plugin(user_id=user1.id, plugin_id=plugin.id)
            UserPluginRepository.create_user_plugin(user_id=user2.id, plugin_id=plugin.id)
