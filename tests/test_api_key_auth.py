...
class TestAPIKeyStorage:
    ...
    def test_create_api_key_in_database(self, authenticated_user):
        ...
        db_key = APIKey.create(user_id=user_id, name="Test Key", key_hash=key_hash, key_prefix=key_prefix)
    ...
    def test_retrieve_api_key_by_prefix(self, authenticated_user):
        ...
        APIKey.create(user_id=user_id, name="Test Key", key_hash=key_hash, key_prefix=key_prefix)

        # Retrieve by prefix
        db_key = APIKey.select().where((APIKey.key_prefix == key_prefix) & (APIKey.is_active)).first()
...
class TestAPIKeyVerification:
    ...
    def test_verify_inactive_api_key(self, authenticated_user):
        ...
        # Create inactive key
        APIKey.create(user_id=user_id, name="Test Key", key_hash=key_hash, key_prefix=key_prefix, is_active=False)
...
class TestAPIKeyEndpoints:
    ...
    def test_create_api_key_via_endpoint(self, authenticated_user):
        ...
        response = authenticated_user.post("/api/keys", json={"name": "CLI Tool"})
...
class TestAPIKeyAuthentication:
    ...
    def test_reject_invalid_api_key_header(self, client):
        ...
        response = client.get("/api/keys", headers={"Authorization": "ApiKey invalid_key_12345"})
    ...
    def test_authenticate_with_cookie_and_api_key(self, authenticated_user, client):
        ...
        key_response = client.get("/api/keys", headers={"Authorization": f"ApiKey {api_key}"})
