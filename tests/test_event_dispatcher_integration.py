...
class TestEventDispatcherIntegration:
    ...
    def test_stage_transition_event_updates_database(self, peewee_test_db, app, real_dispatcher):
        ...
        with app.app_context():
            ...
            event = JobEvent(job_id=job_id, step="stage_1_complete", message="Stage 1 done", progress=0.33)
            ...
            assert job.current_stage == "code_execution", f"Stage not updated! Got: {job.current_stage}"
            assert job.progress == 0.33, f"Progress not saved! Got: {job.progress}"
            assert job.status == "processing", f"Status not updated! Got: {job.status}"
    ...
    def test_stage_transitions_through_full_pipeline(self, peewee_test_db, app, real_dispatcher):
        ...
        with app.app_context():
            ...
            # Stage 1 transition
            event1 = JobEvent(job_id=job_id, step="stage_1_complete", progress=0.33, message="Stage 1 done")
            ...
            # Stage 2 transition
            event2 = JobEvent(job_id=job_id, step="stage_2_complete", progress=0.66, message="Stage 2 done")
            ...
            # Stage 3 transition
            event3 = JobEvent(job_id=job_id, step="stage_3_complete", progress=0.99, message="Stage 3 done")
    ...
    def test_event_persisted_to_database(self, peewee_test_db, app, real_dispatcher):
        ...
        with app.app_context():
            ...
            # Emit event
            event = JobEvent(job_id=job_id, step="test_step", message="Test message", progress=0.5)
            real_dispatcher.emit(event)
    ...
    def test_non_stage_events_dont_update_job_stage(self, peewee_test_db, app, real_dispatcher):
        ...
        with app.app_context():
            ...
            event = JobEvent(job_id=job_id, step="extracting_pdf", message="Extracting...", progress=0.1)
            real_dispatcher.emit(event)
    ...
    def test_chat_events_not_persisted(self, peewee_test_db, app, real_dispatcher):
        ...
        with app.app_context():
            ...
            event = JobEvent(job_id=job_id, step="chat_response", content="User asked...", progress=None)
            ...
            assert events_after == events_before, "Chat event should not be persisted"
    ...
    def test_dispatcher_dependency_injection(self, peewee_test_db, app):
        ...
        with app.app_context():
            ...
            job = Job.get_by_id(job_id)
            assert job.progress == 0.33, "Dispatcher not properly configured - job progress not updated!"
