...
def mock_upload_externals(app):
    ...
    with (
        patch("utils.pdf_utils.extract_page_count", return_value=5),
...
def test_user_credentials():
    ...
    return {"username": "testuser", "email": "testuser@example.com", "password": "TestPassword123!"}
...
def create_test_user(client, app):
    ...
    def _create_user(username, email, password, is_active=True):
        with app.app_context():
            ...
            user = User.create(username=username, email=email, password_hash=password_hash, is_active=is_active)
            return user.id
...
def test_user(app, create_test_user, test_user_credentials, request):
    ...
    user_id = create_test_user(username, email, test_user_credentials["password"], is_active=True)
...
def inactive_user(app, create_test_user, request):
    ...
    user_id = create_test_user(username, email, "InactivePass123!", is_active=False)

    return {"id": user_id, "username": username, "email": email, "is_active": False}
...
def active_user(app, create_test_user, request):
    ...
    user_id = create_test_user(username, email, "ActivePass123!", is_active=True)

    return {"id": user_id, "username": username, "email": email, "is_active": True}
...
def multiple_users(client, app, create_test_user):
    ...
    users = []
    for i in range(3):
        user_id = create_test_user(f"user{i + 1}", f"user{i + 1}@example.com", f"Password{i + 1}!", is_active=True)
        users.append(
            {
                "id": user_id,
                "username": f"user{i + 1}",
...
def authenticated_client_with_fixtures(client, app, create_test_user, peewee_test_db):
    ...
    """
    user_id = create_test_user("testuser", "testuser@example.com", "TestPassword123!", is_active=True)
...
def other_user(app, create_test_user):
    ...
    user_id = create_test_user("otheruser", "otheruser@example.com", "OtherPassword123!", is_active=True)
...
def test_job_with_chat(authenticated_user, create_test_job, app):
    ...
    with app.app_context():
        ...
        ChatMessage.create(session=chat_session, role="user", content="What are the main findings of this paper?")
        ChatMessage.create(session=chat_session, role="assistant", content="The main findings are...")
