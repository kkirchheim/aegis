...
class TestJobService:
    ...
    def test_create_job_failure(self):
        ...
        with patch.object(job_service, "Job") as mock_job:
            ...
            result = job_service.create_job(
                job_id="job123", pdf_path="/path/to/file.pdf", pdf_filename="file.pdf", user_id=1
            )
    ...
    def test_update_job_status_success(self):
        ...
        with patch.object(job_service, "Job") as mock_job_cls:
            mock_job = MagicMock(id="job123", progress=0.0, status="pending", current_stage="pending")
            ...
            result = job_service.update_job_status(
                job_id="job123", status="processing", progress=0.5, current_stage="stage1"
            )
    ...
    def test_update_job_completion(self):
        ...
        with patch.object(job_service, "Job") as mock_job_cls:
            mock_job = MagicMock(id="job123", status="processing", current_stage="processing")
            mock_job_cls.get_by_id.return_value = mock_job

            result = job_service.update_job_completion(job_id="job123", report={"status": "success"})
    ...
    def test_delete_job_success(self):
        ...
        with (
            patch.object(job_service, "JobRepository") as mock_repo,
...
class TestEventDispatcher:
    ...
    def test_emit_event_persists_to_database(self):
        ...
        event = JobEvent(job_id="job123", step="test_step", message="Test message", severity="info", progress=0.5)
    ...
    def test_emit_event_handles_stage_transitions(self):
        ...
        event = JobEvent(job_id="job123", step="pdf_analysis_complete", message="PDF analysis complete", progress=0.33)
    ...
    def test_emit_event_logs_progress(self):
        ...
        event = JobEvent(job_id="job123", step="test_step", message="Test", progress=0.75)
...
class TestCacheService:
    ...
    def test_get_cached_paper_analysis_hit(self):
        ...
        with (
            patch("services.cache_service.Config") as mock_config,
    ...
    def test_get_cache_stats(self):
        ...
        with (
            patch.object(cache_service, "ExecutionDetails") as mock_exec,
...
class TestAnalysisService:
    ...
    def test_extract_and_analyze_pdf_success(self):
        ...
        with (
            patch.object(analysis_service, "extract_pdf_text") as mock_extract,
            patch.object(analysis_service, "parse_paper_with_claude") as mock_parse,
            patch.object(analysis_service, "store_paper_analysis"),
            patch("services.analysis_service.Config") as mock_config,
        ):
            mock_config.ENABLE_CACHING = False
            mock_extract.return_value = "PDF text content"
            mock_parse.return_value = {"title": "Test Paper", "abstract": "Abstract", "citations": [], "artifacts": []}

            mock_llm = Mock()
            result = analysis_service.extract_and_analyze_pdf(
                pdf_path="/path/file.pdf", job_id="job123", llm_provider=mock_llm
            )
    ...
    def test_parse_paper_with_claude(self):
        ...
        response_json = json.dumps(
            {"title": "Test Paper", "abstract": "Abstract text", "citations": [], "artifacts": []}
        )
        mock_llm.complete.return_value = response_json

        result = analysis_service.parse_paper_with_claude(pdf_text="Some PDF content", llm_provider=mock_llm)
    ...
    def test_parse_paper_with_claude_markdown_json(self):
        ...
        response_json = json.dumps({"title": "Test Paper", "abstract": "Abstract", "citations": [], "artifacts": []})
    ...
    def test_store_paper_analysis(self):
        ...
        with patch.object(analysis_service, "PaperAnalysis") as mock_model:
            ...
            analysis_service.store_paper_analysis(job_id="job123", paper_info=paper_info, pdf_text="text content")
...
class TestEvaluationService:
    ...
    def test_evaluate_reproducibility_plugins_success(self):
        ...
        with (
            patch.object(evaluation_service, "PaperAnalysisRepository") as mock_paper_repo,
            ...
            result = evaluation_service.evaluate_reproducibility_plugins(job_id="job123", llm_provider=mock_llm)
        ...
        with patch.dict("os.environ", {"OLLAMA_BASE_URL": "http://localhost:11434"}):
            with patch("llm.ollama_provider.requests.get") as mock_get:
                ...
                with patch("llm.ollama_provider.requests.post") as mock_post:
                    ...
                    mock_post_response.json.return_value = {"message": {"content": "Response text"}}
...
class TestPipelineOrchestrator:
    ...
    def test_orchestrator_emit_event(self):
        ...
        orchestrator.emit_event(job_id="job123", step="test_step", message="Test message", progress=0.5)
