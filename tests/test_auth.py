...
class TestPasswordHashing:
    ...
    def test_verify_password_correct(self):
        ...
        assert verify_password(password, hash_result), "Correct password should verify"
    ...
    def test_verify_password_incorrect(self):
        ...
        assert not verify_password("wrongpassword", hash_result), "Wrong password should NOT verify"
        assert not verify_password("", hash_result), "Empty password should NOT verify"
    ...
    def test_verify_password_invalid_hash(self):
        ...
        # Invalid format (no $)
        assert not verify_password("test", "invalido"), "Invalid hash should not verify"

        # Invalid format (too many $)
        assert not verify_password("test", "a$b$c"), "Invalid hash should not verify"
...
class TestPasswordReset:
    ...
    def test_password_reset_creates_new_hash(self):
        ...
        assert verify_password(old_password, old_hash), "Old password should verify with old hash"

        # Verify new password does NOT work with old hash
        assert not verify_password(new_password, old_hash), "New password should NOT verify with old hash"

        # Verify new password works with new hash
        assert verify_password(new_password, new_hash), "New password should verify with new hash"

        # Verify old password does NOT work with new hash
        assert not verify_password(old_password, new_hash), "Old password should NOT verify with new hash"
...
class TestPasswordPBKDF2:
    ...
    def test_pbkdf2_parameters(self):
        ...
        # Should match
        assert computed_hash.hex() == hexhash, "PBKDF2 computation should match stored hash"
