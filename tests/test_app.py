...
class TestEventEmission:
    ...
    def test_emit_event_stores_in_database(self, app):
        ...
        with app.app_context():
            ...
            c = conn.cursor()
            c.execute(
                "INSERT INTO jobs (id, status, pdf_path, current_stage, progress, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            ...
            # Emit an event
            emit_event(job_id, {"step": "test_step", "message": "Test message", "progress": 50})
...
class TestStageEvents:
    ...
    def test_stage_1_events(self, app):
        ...
        with app.app_context():
            ...
            emit_event(
                job_id,
                {"step": "stage_1_starting", "stage": "paper_analysis", "message": "Stage 1 starting", "progress": 5},
            )
            ...
            conn = get_db()
            c = conn.cursor()
            c.execute("SELECT COUNT(*) as count FROM events WHERE job_id = ? AND step LIKE 'stage_1%'", (job_id,))
    ...
    def test_all_three_stages_events(self, app):
        ...
        with app.app_context():
            ...
            for step, stage, progress in stages:
                emit_event(job_id, {"step": step, "stage": stage, "message": f"{step}", "progress": progress})
            ...
            for stage_num in [1, 2, 3]:
                c.execute(
                    f"SELECT COUNT(*) as count FROM events WHERE job_id = ? AND step LIKE 'stage_{stage_num}%'",
                    (job_id,),
                )
                count = c.fetchone()["count"]
...
class TestErrorHandling:
    ...
    def test_agent_think_missing_job_id(self, authenticated_user):
        ...
        response = authenticated_user.post(
            "/api/agent/think", data=json.dumps({"repo_state": {}}), content_type="application/json"
        )
    ...
    def test_agent_log_missing_job_id(self, authenticated_user):
        ...
        response = authenticated_user.post(
            "/api/agent/log", data=json.dumps({"message": "test"}), content_type="application/json"
        )
        assert response.status_code in (400, 422)
    ...
    def test_agent_execution_missing_job_id(self, authenticated_user):
        ...
        response = authenticated_user.post(
            "/api/agent/execution",
            data=json.dumps({"commands_run": "test", "stdout_combined": "test"}),
