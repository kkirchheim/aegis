...
class PluginCreateSchema(Schema):
    ...
    name = fields.Str(
        ...
        description="Plugin name (1-255 characters)"
    )
...
class ActivatePluginSchema(Schema):
    ...
    is_active = fields.Bool(
        required=True,
        description="Whether plugin should be active"
    )
...
class PluginSchema(Schema):
    ...
    is_default = fields.Bool(
        description="Whether this is a default system plugin"
    )
    created_at = fields.DateTime(
        description="When plugin was created"
    )
...
class UserPluginSchema(Schema):
    ...
    id = fields.UUID(
        description="Unique user-plugin identifier"
    )
    plugin_id = fields.UUID(
        description="Reference to the global plugin"
    )
    name = fields.Str(
        description="Plugin name"
    )
    ...
    is_active = fields.Bool(
        description="Whether this plugin is active for the user"
    )
    custom_prompt = fields.Str(
        allow_none=True,
    ...
    created_at = fields.DateTime(
        description="When user activated this plugin"
    )
...
class PluginListSchema(Schema):
    ...
    plugins = fields.List(
        fields.Nested(UserPluginSchema),
        description="List of user plugins"
    )
    total = fields.Int(
        description="Total number of plugins for user"
    )
