...
class JobSchema(Schema):
    ...
    current_stage = fields.Str(
        allow_none=True,
        description="Current processing stage (e.g., 'uploading', 'extracting', 'analyzing')"
    )
...
class JobDetailSchema(Schema):
    ...
    events = fields.List(
        fields.Dict(),
        allow_none=True,
        description="List of processing events with timestamps and details"
    )
    paper_analysis = fields.Dict(
        allow_none=True,
        description="Extracted paper analysis including metadata, abstract, findings"
    )
    artifacts = fields.List(
        fields.Dict(),
        allow_none=True,
        description="Generated artifacts (images, tables, supplementary materials)"
    )
    report = fields.Dict(
        allow_none=True,
...
class JobUploadSchema(Schema):
    ...
    file = fields.Field(
        required=True,
        load_only=True,
        description="PDF file to process (multipart form upload)"
    )
